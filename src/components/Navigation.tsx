      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center h-full py-1">
            ...
            fontWeight: 'bold'
          }} className=" scale-120 transition-all duration-300 hover:scale-150 " />
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map(item => <Link key={item.label} to={item.href} className={`text-black hover:text-primary transition-colors duration-300 font-medium whitespace-nowrap ${location.pathname === item.href ? 'text-primary' : ''}`}>
                {item.label}
              </Link>)}
